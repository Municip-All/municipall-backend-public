...
export class AiEngineService {
  ...
  constructor(private configService: ConfigService) {
    ...
    if (apiKey && apiKey !== 'sk-proj-xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx') {
      ...
      this.logger.warn('OPENAI_API_KEY is missing or using placeholder. AI features will be disabled.');
    }
  }

  async classifyReport(text: string): Promise<string> {
    if (!this.openai) {
  ...
  async generateSentimentSummary(reports: any[]): Promise<string> {
    if (!this.openai) {
